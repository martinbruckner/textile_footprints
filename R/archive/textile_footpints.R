...
write = TRUE
plot_dir = "output" # set plot directory
...
# load FABIO data
Y <- readRDS(file=paste0("/mnt/nfs_fineprint/tmp/fabio/v",vers,"/Y.rds"))
...
items_group <- as.data.table(openxlsx::read.xlsx("input/items_conc.xlsx", sheet = ifelse(vers == "1.1", "items_german_1.1", "items_german")))
if (lang == "de") {
...
Y <- merge(cbind(fabio_index,as.data.table(as.matrix(Y[[as.character(yr)]]))), 
           items_group[,.(item_code,comm_group_plot)], by = c("item_code"), all.x = TRUE, sort = FALSE)
setkey(Y, area_code, comm_code)

# # colors for food groups
# food_cols <- openxlsx::read.xlsx("input/items_conc.xlsx", sheet = "colors_alt", colNames = FALSE)
...
fp_sq <- footprint(country = "AUT",  allocation = "value", year = yr, y = Y$food_t_pc, X = X, E = E_all, index = fabio_index, v = vers, take.result = take_prod_result, result.dir = "data", result.suffix = "sq")
