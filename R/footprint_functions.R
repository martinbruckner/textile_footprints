...
footprint <- function(country = "AUT", consumption = "food", allocation = "value", 
  ...
  if (!take.result){
    ...
    saveRDS(FP, paste0(result.dir,"/v",vers,"/FP_production_",result.suffix,"_",year,".rds"))
  } else if (file.exists(paste0(result.dir,"/v",vers,"/FP_production_",result.suffix,"_",year,".rds"))){
    FP <- readRDS(paste0(result.dir,"/v",vers,"/FP_production_",result.suffix,"_",year,".rds"))
  } else {
    stop("no production footprint result exsiting under", paste0(result.dir,"/v",vers,"/FP_production_",result.suffix,"_",year,".rds"))
  }
  FP <- as(FP, "dgTMatrix")
...
tex_footprint <- function(Y = Y_EU, consumer = "EU", L = L, E_int = E_int, 
                          fabio_index = fabio_index, exio_index = exio_index, 
                          agg_by = c("consumer", "continent_origin", "group_origin", "group_target")){
  # compute footprints
  FP <- t(t(L) * Y)
  rownames(FP) <- short_index <- paste0(fabio_index$iso3c, "_", fabio_index$item_code)
  colnames(FP) <- long_index <- paste0(exio_index$iso3c, "_", exio_index$item_code)
  # p17 C_TEXT  Textiles (17)
  # p18 C_GARM  Wearing apparel; furs (18)
  # p19 C_LETH  Leather and leather products (19)
  FP <- FP[, exio_index$item_code %in% c("C_TEXT", "C_GARM", "C_LETH")]
  FP <- as(FP, "dgTMatrix")
  results <- data.table(origin=rownames(FP)[FP@i + 1], target=colnames(FP)[FP@j + 1], production =FP@x)
  results[,`:=`(country_origin = substr(origin,1,3),
                item_code_origin = substr(origin,5,100),
                country_target = substr(target,1,3),
                item_code_target = substr(target,5,100),
                consumer = consumer)]
  ...
  # calculate environmental footprints for all extensions from production footprint
  match_origin = match(results$origin, short_index)
  match_target = match(results$target, long_index)
  ...
  # add auxiliary info on origin and target commodities
  results[,`:=`(
    item_origin = fabio_index$item[match_origin],
    group_origin  = fabio_index$comm_group[match_origin],
    item_target = exio_index$item[match_target],
    group_target  = exio_index$comm_group[match_target],
    iso_origin = fabio_index$iso3c[match_origin],
    continent_origin  = fabio_index$continent[match_origin])]
  
  # reorder columns
  results <- results %>%
    relocate(c(production, landuse:ghg_other), .after = continent_origin) %>%
    relocate(c(iso_origin, continent_origin), .after = country_origin) %>%
    relocate(item_origin:group_origin, .after = item_code_origin) %>%
    relocate(item_target:group_target, .after = item_code_target) 
  
  if(!missing(agg_by)){
    
    results <- results[, lapply(.SD, sum, na.rm=TRUE), 
                       by = agg_by, 
                       .SDcols = c("production", "landuse", "biomass", "green", "blue", "ghg", "luh", "ghg_energy", "ghg_live", "ghg_other", "ghg_all", "biodiv", "n_application", "p_application")]
  }
}


footprint_hybrid <- function(L, Y, consumer, category,  E_int, fabio_index, exio_index, agg_by){
  
  Y_target <- Y[[paste0(consumer,"_",category)]]#, with = FALSE])
  if(!all(names(fabio_index == names(exio_index)))){
    stop("fabio and exiobase index need to be of same format")
  } else{
    hybrid_index <- rbind(fabio_index, exio_index)
  }
  ...
  FP <- t(t(L) * Y_target)
  rownames(FP) <- short_index <- paste0(fabio_index$iso3c, "_", fabio_index$item_code)
  colnames(FP) <- long_index <- paste0(hybrid_index$iso3c, "_", hybrid_index$item_code)
  #  saveRDS(FP, paste0(result.dir,"/v",vers,"/FP_production_",result.suffix,"_",year,".rds"))
  ...
  results[,`:=`(
                item_origin = fabio_index$item[match_origin],
                group_origin  = fabio_index$comm_group[match_origin],
                item_target = hybrid_index$item[match_target],
                group_target  = hybrid_index$comm_group[match_target],
                iso_origin = fabio_index$iso3c[match_origin],
                continent_origin  = fabio_index$continent[match_origin])]
...
fp_map <- function(fp, map = world_map, indicator = "landuse", per_capita = FALSE,
  ...
  if (lang == "de") {
  indicators_long <-  c("landuse" = "Anbaufläche", "biomass" = "Biomasse", "blue" = "Süßwassereinsatz", "green" = "Grünes Wasser", "ghg" = "THG-Emissionen", "luh" = "THG-Emissionen", "ghg_all" = "THG-Emissionen",
  ...
  indicators_long <- c("landuse" = "Cropland", "biomass" = "Biomass", "blue" = "Freshwater use", "green" = "Green water use", "ghg" = "GHG emissions", "luh" = "GHG emissions", "ghg_all" = "GHG emissions",
                        "biodiv" = "Biodiversity loss", "n_application" = "Nitrogen application", "p_application" = "Phosphorous application")
...
fp_mosaic <- function(fp, indicator, per_captia = FALSE, consumer_country = "AUT",
  ...
  if (lang == "de") {
    ...
    axis_labs <- c("landuse" = expression(paste("Anbaufläche in ", m^2)), 
                   "biomass" =  "", 
                   "blue" = expression(paste("Wassereinsatz in ", m^3)), 
                   "green" = "Grünes Wasser", 
    ...
    axis_labs <- list("landuse" = expression(paste("Cropland in ", m^2)), 
                   "biomass" =  "", 
                   "blue" = expression(paste("Freshwater use in ", m^3)), 
                   "green" = "Grünes Wasser", 
                   ...
                   "n_application" = "Nitrogen application in g", 
                   "p_application" = "Phosphorous application in g")
  }
...
stacked_bars <- function(fp_list, indicator = "landuse", per_capita = FALSE,
  ...
  if (lang == "en") diet_labs = c("sq" = "Status \nQuo", "epo" = "Food\nPyramid", "eat" =  "Austrian \nPlanetary \nHealth Diet", "epo2" = "Food\nPyramid \n2.0", "epoV" = "Vegetarian\nFood\nPyramid \n2.0", "epoVV" = "Vegan\nFood\nPyramid \n2.0")
    ...
    if(!is.na(omit)) tab <- filter(tab, comm_group_plot != omit)
    #tab <- filter(tab, get(indicator) > 0)
    return(tab)
    }) %>% rbindlist
...
stacked_bars_general <- function(fp_list, bounds, indicator = "landuse", exclude_grazing = TRUE, cols = cols_vect,
                         aggregate_by, target,
                         origin_items = "ALL", target_items = "ALL", origin_groups, target_groups, 
                         axis_lab = "", leg_name_group = NULL, leg_name_bound = NULL,
                         title = "",
                         lang = "de",
                         reverse_legend = FALSE, omit = NA){
  
  # aggregate footprints by consumer item and combine to table with target groups in columns
  
  fp_agg_long <- lapply(names(fp_list), function(fp){
    if (indicator == "landuse" & exclude_grazing) fp_list[[fp]] <- fp_list[[fp]][group_origin != "Grazing",]
    tab <- fp_aggregate(fp_list[[fp]], aggregate_by = aggregate_by, indicators = indicator)%>%
      #rename(!!sym(fp) := !!sym(indicator)) %>% 
      rename(group = !!sym(target))
    if(indicator == "ghg") tab <- dplyr::select(tab, !ghg_all)
    if(!is.na(omit)) tab <- filter(tab, comm_group_plot != omit)
    #tab <- filter(tab, get(indicator) > 0)
    return(tab)
  ...
  # plot
  cols_sel <- cols[names(cols) %in% unique(fp_agg_long$category)]
  
  fp_plot <- ggplot(fp_agg_long, aes(x = group, y = !!sym(indicator), fill = factor(category, levels = rev(names(cols_sel))))) +
    geom_bar(stat="identity", alpha = 0.85) +
    scale_fill_manual(values = cols_sel, name = leg_name_group, guide = guide_legend(reverse = reverse_legend)) +
    labs(y = axis_lab, x = "") +
    theme_minimal() +
  ...
  if(!missing(bounds)) {
    if(lang == "de") {
      fp_plot <- fp_plot + 
        geom_hline(aes(yintercept = bounds[ifelse(indicator %in% c("ghg", "luh", "ghg_pb"), "ghg_all", indicator), "boundary"], color = "Planetare Belastungsgrenze"), size = 0.4, linetype = "solid") +
        geom_hline(aes(yintercept = bounds[ifelse(indicator %in% c("ghg", "luh", "ghg_pb"), "ghg_all", indicator), "upper"], color = "Obergrenze der Unsicherheitszone"), size = 1.0) +
        geom_hline(aes(yintercept = bounds[ifelse(indicator %in% c("ghg", "luh", "ghg_pb"), "ghg_all", indicator), "lower"], color = "Untergrenze der Unsicherheitszone"), size = 1.0)+
        scale_color_manual(values = c("Planetare Belastungsgrenze" = "black",  "Obergrenze der Unsicherheitszone" = "red", "Untergrenze der Unsicherheitszone" = "darkgreen")) + 
        #scale_linetype_manual(values = c("Planetare Belastungsgrenze" = "solid",  "Obergrenze der Unsicherheitszone" = "solid", "Untergrenze der Unsicherheitszone" = "solid")) + 
        labs(color = leg_name_bound)+
        guides(color=guide_legend(direction='vertical'))
    } else if (lang == "en"){
      fp_plot <- fp_plot + 
        geom_hline(aes(yintercept = bounds[ifelse(indicator %in% c("ghg", "luh", "ghg_pb"), "ghg_all", indicator), "boundary"], color = "Planetary boundary"), size = 0.4, linetype = "solid") +
        geom_hline(aes(yintercept = bounds[ifelse(indicator %in% c("ghg", "luh", "ghg_pb"), "ghg_all", indicator), "upper"], color = "Upper limit of zone of unceartainty"), size = 1.0) +
        geom_hline(aes(yintercept = bounds[ifelse(indicator %in% c("ghg", "luh", "ghg_pb"), "ghg_all", indicator), "lower"], color = "Lower limit of zone of unceartainty"), size = 1.0)+
        scale_color_manual(values = c("Planetary boundary" = "black",  "Upper limit of zone of unceartainty" = "red", "Lower limit of zone of unceartainty" = "darkgreen")) + 
        #scale_linetype_manual(values = c("Planetare Belastungsgrenze" = "solid",  "Obergrenze der Unsicherheitszone" = "solid", "Untergrenze der Unsicherheitszone" = "solid")) + 
        labs(color = leg_name_bound)+
        guides(color=guide_legend(direction='vertical', order = 2), fill = guide_legend(order = 1, reverse = reverse_legend))
    }
  }
...
pb_bars <- function(fp_agg, indicator, lang = "de", xlabsize = 8) {
  ...
  if (lang == "de"){
    ...
    ylabs <-c(
      "landuse" = "Flächennutzung in in m<sup>2</sup>", 
      "biomass" = "Biomasse", 
      "blue" = "Wasserverbrauch in m<sup>3</sup>", 
      "green" = "Grüner Wasserverbrauch in m<sup>3</sup>", 
      "ghg_pb" = "THG-Emissionen (exkl. Energie und LUC) in t CO<sub>2</sub>-Äq.",  
    ...
    ylabs <-c(
      ...
      "n_application" = "Nitrogen application in kg", 
  ...
  pb_bar <- ggplot(fp_agg, aes(x = diet, y = !!sym(indicator))) + 
   ...
   theme_minimal()+
   theme(axis.title.y = element_markdown(), axis.text.x = element_text(size = xlabsize))
...
circle_plot <- function(fp_table, diet, ylim.max = 4, ylim.min = 0, log = FALSE, legend = TRUE){
  ...
  title = ifelse(diet %in% c("sq", "Status Quo"),"Status Quo", ifelse(diet %in% c("eat", "Planetary Health"),"Österreichische Planetary Health Diet",ifelse(diet %in% c("epo", "Ernährungspyramide"),"Ernährungspyramide",NA)))

  
  fp_circle_scen <- data.frame(ind = as.character(names(fp_table[-1])), #as.factor(names(fp_table[-1]), levels = names(fp_table[-1]))
...
circle_plot_grad <- function(fp_table, diet, ylim.max = 4, ylim.min = 0, log = FALSE, legend = TRUE, lang = "de", offset = 0, titlesize = NULL, labsize = NULL){ # expandlim = NULL
  
  if (lang == "de"){
    
    fp_table <- rename(fp_table, Flächennutzung = landuse, Süßwassereinsatz = blue, `THG-Emissionen` = ghg_all, Biodiversitätsverlust = biodiv, Stickstoffeinsatz = n_application, Phosphoreinsatz = p_application)
    ...
    diet_index <- ifelse(diet %in% c("sq", "Status Quo"),1, 
                         ifelse(diet %in% c("eat", "Planetary Health"),3,
                                ifelse(diet %in% c("epo", "Ernährungspyramide"),2,
                                       ifelse(diet %in% c("epo2", "Ernährungspyramide 2.0"),4,
                                              ifelse(diet %in% c("epoV", "Vegetarische Ernährungspyramide 2.0"),5,
                                                     ifelse(diet %in% c("epoVV", "Vegane Ernährungspyramide 2.0"),6,NA))))))
    title = ifelse(diet %in% c("sq", "Status Quo"),"Status Quo",
                   ifelse(diet %in% c("eat", "Planetary Health"),"Österreichische Planetary Health Diet",
                          ifelse(diet %in% c("epo", "Ernährungspyramide"),"Ernährungspyramide",
                                 ifelse(diet %in% c("epo2"),"Ernährungspyramide 2.0",
                                        ifelse(diet %in% c("epoV"),"Vegetarische \nErnährungspyramide 2.0",
                                               ifelse(diet %in% c("epoVV"),"Vegane \nErnährungspyramide 2.0",NA))))))
    line_cols <- c( "darkgreen", "darkred")
    names(line_cols) <- c("Untergrenze der Unsicherheitszone", "Obergrenze der Unsicherheitszone")
  } else if (lang == "en"){
    
    fp_table <- rename(fp_table, `Cropland \nuse` = landuse, `Freshwater \nuse` = blue, `GHG \nemissions` = ghg_all, `Biodiversity \nloss` = biodiv, `Nitrogen \napplication` = n_application, `Phosphorus \napplication` = p_application)
    
    diet_index <- ifelse(diet %in% c("sq", "Status Quo"),1, 
                         ifelse(diet %in% c("eat", "Planetary Health"),3,
                                ifelse(diet %in% c("epo", "Food Pyramid"),2,
                                       ifelse(diet %in% c("epo2", "Food Pyramid 2.0"),4,
                                              ifelse(diet %in% c("epoV", "Vegetarian Food Pyramid 2.0"),5,
                                                     ifelse(diet %in% c("epoVV", "Vegan Food Pyramid 2.0"),6,NA))))))
    title = ifelse(diet %in% c("sq", "Status Quo"),"Status Quo", 
                   ifelse(diet %in% c("eat", "Planetary Health"),"Austrian Planetary Health Diet",
                          ifelse(diet %in% c("epo", "Food Pyramid"),"Food Pyramid",
                                 ifelse(diet %in% c("epo2", "Food Pyramid 2.0"),"Food Pyramid 2.0",
                                        ifelse(diet %in% c("epoV", "Vegetarian Food Pyramid 2.0"),"Vegetarian \nFood Pyramid 2.0",
                                               ifelse(diet %in% c("epoVV", "Vegan Food Pyramid 2.0"),"Vegan \nFood Pyramid 2.0",NA))))))
    line_cols <- c( "darkgreen", "darkred")
    names(line_cols) <- c("Lower limit of zone of unceartainty", "Upper limit of zone of unceartainty")
  }
  
  fp_circle_scen <- data.frame(ind = factor(names(fp_table[-1]), levels = names(fp_table[-1])), 
                               value = unlist(fp_table[diet_index,][-1]),
                               limit = ifelse(unlist(fp_table[diet_index,])[-1] > ifelse(log, 0, 1), "yes", "no"))
  ...
  plt <- ggplot(fp_circle_scen_exp) +
   ...
   # ) + 
    ...
    coord_polar(clip = "off", start = offset)+
    labs(title = title, color = NULL) + 
    #expand_limits(y = expandlim) + # or some other arbitrarily large number
    theme_minimal() +
    theme(
      # Remove axis ticks and text
      axis.title = element_blank(),
      ...
      axis.text.y = element_blank(),
      # Use gray text for the region names
      axis.text.x = element_text(color = "gray12", size = labsize, face="bold"), #  hjust = 0, vjust = -1 , vjust = "inward"
      # Move the legend to the bottom
      legend.position = "bottom",
      panel.grid=element_blank(),
      plot.title = element_text(hjust = 0.5, vjust = 0.1,  size = titlesize, face = "bold", margin = margin(0,0,0.7,0, unit = "cm")),
...
circle_plot_grad1 <- function(fp_table, diet, ylim.max = 4, ylim.min = 0, log = FALSE, lab_pos = 4,
                              legend = TRUE, lang = "de", offset = 0, titlesize = NULL, labsize = NULL){ # expandlim = NULL
  ...
  plt <- ggplot(fp_circle_scen_exp) +
    ...
    geom_text(aes(x = ind, y = lab_pos, label = ind), check_overlap = TRUE,
              color = "gray12",  fontface = "bold", size = labsize) +
    coord_polar(clip = "off", start = offset)+
    labs(title = title, color = NULL) + 
    #expand_limits(y = expandlim) + # or some other arbitrarily large number
    theme_minimal() +
    theme(
      ...
      axis.text.y = element_blank(),
      # Use gray text for the region names
      axis.text.x = element_blank(),#element_text(color = "gray12", size = labsize, face="bold"), #  hjust = 0, vjust = -1 
      # Move the legend to the bottom
      legend.position = "bottom",
      panel.grid=element_blank(),
      plot.title = element_text(hjust = 0.5, vjust = 0.0,  size = titlesize, face = "bold", margin = margin(0,0,0.2,0, unit = "cm")),
      plot.margin = margin(0.5,1,0.5,1,unit = "cm"),
      legend.box.just = "center",
      legend.title = element_blank(),
      legend.title.align = 0.5,
      legend.margin=margin(0.2,0,0,0.5,unit = "cm")
    )
...
## under construction: a version with the planetary boundary instead of upper/lower limits
circle_plot_grad_pb <- function(fp_table, diet, ylim.max = 4, ylim.min = 0, log = FALSE, legend = TRUE, lang = "de"){
  ...
  if (lang == "de"){
    ...
    #names(line_cols) <- c("Untergrenze der Unsicherheitszone", "Obergrenze der Unsicherheitszone")
    names(line_cols) <- c("Planetare Grenze")
    
  } else if (lang == "en"){
    diet_index <- ifelse(diet %in% c("sq", "Status Quo"),1, 
                         ifelse(diet %in% c("eat", "Planetary Health"),3,
                                ifelse(diet %in% c("epo", "Food Pyramid"),2,
                                       ifelse(diet %in% c("epo2", "Food Pyramid 2.0"),4,NA))))
    title = ifelse(diet %in% c("sq", "Status Quo"),"Status Quo", 
                   ifelse(diet %in% c("eat", "Planetary Health"),"Austrian Planetary Health Diet",
                          ifelse(diet %in% c("epo", "Food Pyramid"),"Food Pyramid",
                                 ifelse(diet %in% c("epo2", "Food Pyramid 2.0"),"Food Pyramid 2.0", NA))))
    #line_cols <- c( "darkgreen", "darkred")
    line_cols <- c( "black")
    #names(line_cols) <- c("Lower limit of zone of unceartainty", "Upper limit of zone of unceartainty")
    names(line_cols) <- c("Planetary Boundary")
...
# generalized version
circle_plot_general <- function(fp_table, target, ylim.max = 4, ylim.min = 0, log = FALSE, step = 0.01, lab_pos = 4,
                              legend = TRUE, lang = "de", offset = 0, title = "", titlesize = NULL, labsize = NULL){ # expandlim = NULL
  ...
  fp_circle_scen <- data.frame(ind = factor(names(fp_table[-1]), levels = names(fp_table[-1])), 
                               value = unlist(fp_table[fp_table$group == target,][-1]),
                               limit = ifelse(unlist(fp_table[fp_table$group == target,])[-1] > ifelse(log, 0, 1), "yes", "no"))
  
  
  fp_circle_scen_exp <- fp_circle_scen %>%
    rowwise() %>%
    summarise(ind = ind,
              value = list(seq(value,ylim.min, by = -step))) %>% # seq(value,0, by = -0.01)
  ...
  plt <- ggplot(fp_circle_scen_exp) +
    ...
    #scale_fill_manual(values = c("yes" = "red", "no" = "green")) +
    scale_fill_gradientn(colors = c("white", "green4", "chartreuse", "yellow", "orange", "red", "firebrick", "transparent"), 
                         values = scales::rescale(c(0, 0.5, 0.9, 1.1, 1.9, 2.1, 3, ylim.max)),
                         limits = c(0, ylim.max),
                         name = "Wert relativ\nzum Grenzwert") + # "Value relative to\nplanetary boundary"
    #scale_fill_gradient2(low = "darkgreen", mid =  "yellow", high =  "firebrick", midpoint = 1.2, limits = c(0,4)) +
...
spiderweb <- function(plot.data,
  ...
  if(lang == "de") plot.data <- rename(fp_agg_sel, Flächennutzung = landuse, Süßwassereinsatz = blue, `THG-Emissionen` = ghg_all, Biodiversitätsverlust = biodiv, Stickstoffeinsatz = n_application, Phosphoreinsatz = p_application)
  if(lang == "en") plot.data <- rename(fp_agg_sel, `Cropland \nuse` = landuse, `Freshwater \nuse` = blue, `GHG \nemissions` = ghg_all, `Biodiversity \nloss` = biodiv, `Nitrogen \napplication` = n_application, `Phosphorus \napplication` = p_application)
